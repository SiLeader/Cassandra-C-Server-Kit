  ...
  try {
    ...
    switch (header.opcode) {
      case types::OpCode::kStartup:
        body_bytes =
            startupCaller(id, header, response_header, opt_body.value());
        break;
      case types::OpCode::kOptions:
        body_bytes = optionsCaller(id, header, response_header);
        break;
      case types::OpCode::kQuery:
        body_bytes = queryCaller(id, header, response_header, opt_body.value());
        break;
      case types::OpCode::kPrepare:
        body_bytes =
            prepareCaller(id, header, response_header, opt_body.value());
        break;
      case types::OpCode::kExecute:
        body_bytes =
            executeCaller(id, header, response_header, opt_body.value());
        break;
...
std::vector<byte> RpcBase::startupCaller(std::uint16_t client_id,
                                         const types::Header& request_header,
...
std::vector<byte> RpcBase::queryCaller(std::uint16_t client_id,
  ...
  QueryParameter parameter{};  // TODO get from body
...
std::vector<byte> RpcBase::executeCaller(std::uint16_t client_id,
                                         ...
                                         types::Header& response_header,
                                         const std::vector<tcp::byte>& body) {
  auto id = convert::toShortBytes(body);
  QueryParameter parameter{};  // TODO get from body
